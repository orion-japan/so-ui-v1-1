...
const uploadRemoteFileInfo = async (url: string): Promise<{
  id: string
  name: string
  mime_type: string
  size: number
  ...
  // TODO: Replace with actual implementation to fetch remote file info
...
export const useFile = (fileConfig: FileUpload) => {
  ...
  const handleLoadFileFromLink = useCallback((url: string) => {
    ...
    // Check if params is not null before using params.token if needed in the future
  ...
  }, [checkSizeLimit, notify, t, handleAddFile, handleUpdateFile, params ? params.token : undefined, fileConfig?.allowed_file_types, fileConfig?.allowed_file_extensions])
